...
## 6. Conversational History-Aware LLM Generation
**Change:** Converted the pipeline into an interactive CLI loop using LangChain's `create_history_aware_retriever`, `create_stuff_documents_chain`, and tracking chat history.
**Reason:** A standard RAG pipeline fails on follow-up questions (e.g., "What does that mean?"). By integrating a history-aware retriever, the system first reformulates the user's latest prompt using the chat history to create a standalone search query. It then retrieves the documents, passes them alongside the conversation history to `gemma3:4b`, and generates a polished answer, enabling a natural, continuous chat experience. It still utilizes the deterministic fallback: if no documents are retrieved, it safely outputs *"I don't have enough information to answer the question based on the provided documents"* without prompting the LLM.
